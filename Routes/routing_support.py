...
# Fetch the default permissions for the SUPER_ADMIN role
def get_default_permissions(role: str):
    # Return permissions for the given role, defaulting to SUPER_ADMIN if role is not found
    return role_permissions.get(role, role_permissions.get(Role.SUPER_ADMIN))

@router.post("/user/creation", summary="Create a new user with SUPER_ADMIN role")
async def create_user(user_in: UserCreate, db: AsyncSession = Depends(get_database_secure_connection)):
    try:
        ...
        # Set the role as SUPER_ADMIN by default (Override any passed role)
        user_role = Role.SUPER_ADMIN

        # Get default permissions for the SUPER_ADMIN role (or role passed in future)
        ...
        new_user = User(
            ...
            role=user_role,  # Role is hardcoded to SUPER_ADMIN
            ...
            permissions=default_permissions  # Assign default permissions for SUPER_ADMIN
        )
